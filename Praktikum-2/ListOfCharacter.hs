...
inverse li
    | null li = []
    | otherwise = konso(last li) (inverse (init li))
...
isUnique :: [Char] -> Char -> Int

-- Realisasi
makeUnique l 
    | isEmpty l = []
    | isUnique l (last l) > 1 = makeUnique (init l)
    | otherwise = konsDot(makeUnique (init l)) (last l)

isUnique l x
    | isEmpty l = 0
    | x == head l = 1 + isUnique (tail l) x
    | otherwise = isUnique (tail l) x
